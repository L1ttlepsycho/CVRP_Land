...
class CVRPTester:
    def __init__(self,
        ...
        self.score_list, self.aug_score_list, self.gap_list, self.aug_gap_list, self.time_list = [], [], [], [], []
    ...
    def run(self):
        ...
        if self.path_list:
            ...
            if self.fine_tune_params['enable']:
                ...
                self.fine_tune_data = self.pack_cvrplib(self.path_list)[start: start + self.fine_tune_params['fine_tune_episodes']]  # load dataset from file
                self._fine_tune_and_test()
            else:
                # test the model on test dataset (zero-shot)
                self._test_on_cvrp(store_res=True)
            print(">> Evaluation on {} finished within {:.2f}s".format(self.tester_params['test_set_path'], time.time() - start_time))

            # save results to file
            with open('res/'+os.path.split(self.tester_params['test_set_path'])[-1], 'wb') as f:
            ...
            # save results to file
            if not os.path.isdir('../../../../../results/mls/omni'):
                os.mkdir('../../../../../results/mls/omni')
            with open('../../../../../results/mls/omni/'+os.path.split(self.tester_params['test_set_path'])[-1], 'wb') as f:
                result = {"score_list": self.score_list, "aug_score_list": self.aug_score_list, "gap_list": self.gap_list, "aug_gap_list": self.aug_gap_list}
                pickle.dump(result, f, pickle.HIGHEST_PROTOCOL)
                print(">> Save final results to {}".format('res/'+os.path.split(self.tester_params['test_set_path'])[-1]))
    ...
    def _test(self, store_res=True):
        ...
        #print('data,ep: ',len(self.test_data),test_num_episode)
        #print(self.test_data)

        test_num_episode=len(self.test_data)# Ningjun Xu modified,delete it if needed
        ...
        while episode < test_num_episode:
            ...
            episode += batch_size
            #print('cal_score: ',all_score)
            #print('opt_score: ',opt_sol)
    ...
    def _test_on_cvrp(self, store_res=True):
        self.time_estimator.reset()
        score_AM, gap_AM = AverageMeter(), AverageMeter()
        aug_score_AM, aug_gap_AM = AverageMeter(), AverageMeter()
        ...
        while episode < test_num_episode:
            ...
            elapsed_time_str, remain_time_str = self.time_estimator.get_est_string(episode, test_num_episode)
            self.logger.info("episode {:3d}/{:3d}, Elapsed[{}], Remain[{}], sol:{:.3f}, score:{:.3f}, aug_score:{:.3f}".format(
                episode, test_num_episode, elapsed_time_str, remain_time_str, opt_sol[0],score, aug_score))
